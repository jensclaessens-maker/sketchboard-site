...
// Admin-only stats: sketch count and total disk usage
app.get('/api/stats', requireAuth, (_req, res) => {
  try {
    const files = fs.readdirSync(IMAGES);
    let bytes = 0;
    for (const f of files) {
      try { bytes += fs.statSync(path.join(IMAGES, f)).size; }
      catch (_) { /* file removed mid-loop, skip */ }
    }
    const sketchCount = readData().sketches.length;
    res.json({ count: sketchCount, files: files.length, bytes });
  } catch (e) {
...
app.post('/api/upload', requireAuth, upload.single('image'), (req, res) => {
  ...
  const data = readData();
  // True top of stack: above the highest existing z, not just the count
  const maxZ = data.sketches.reduce((m, s) => Math.max(m, s.z || 0), -1);
  const sk = {
...
// Update a sketch (move / rename / restack / lock)
